...
cd "/Users/panagiotiskourkoutis/Dixis Project 2/Dixis-Project-1/backend"

# Step 1: Export SQLite data
echo "📦 Exporting SQLite data..."
sqlite3 "/Users/panagiotiskourkoutis/Dixis Project 2/backend/database/database.sqlite" <<'EOF'
.headers on
.mode csv
...
SELECT * FROM users;
.output /tmp/categories.csv
SELECT * FROM categories;
.output /tmp/producers.csv
SELECT * FROM producers;
.output /tmp/orders.csv
SELECT * FROM orders;
.output /tmp/order_items.csv
SELECT * FROM order_items;
.output /tmp/carts.csv
SELECT * FROM carts;
.output /tmp/cart_items.csv
...
# Step 2: Create PostgreSQL schema using seeders approach
echo "🔧 Creating PostgreSQL schema..."

php artisan make:command MigrateFromSqlite --no-interaction 2>/dev/null || true

cat > app/Console/Commands/MigrateFromSqlite.php << 'PHPCMD'
<?php
